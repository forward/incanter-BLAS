...
(defn ^"[[D" coerce [vector2d]
  (let [parts (map (fn [v] (double-array v)) vector2d)]
    (into-array (Class/forName "[D") parts)))
...
(defn log10
  ...
  ([A] (transform-with A #(Math/log10 %) log10)))
  ...
  ([& args]
    (reduce (fn [A B]
              (let [a (if (matrix? A) A (matrix A))
                    ...
                    result (Matrix. (.mmul ^Matrix a ^Matrix b))]
...
(defn solve
  ...
  ([^Matrix A & B]
    (if B
      (Matrix. (.solve A (first B)))
      (Matrix. (.solve A (org.jblas.DoubleMatrix/eye (.columns A)))))))
